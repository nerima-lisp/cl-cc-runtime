;;;; t/async-generators-test.lisp — async iterators + async generators (src/async-generators.lisp).
;;;;
...
(describe
  "async iterators (async-generators.lisp)"
  (it
    "rt-aiter-from-list + rt-aiter-collect round-trips the items"
    (expect
      (cl-cc/runtime::rt-future-await
        (cl-cc/runtime::rt-aiter-collect (cl-cc/runtime::rt-aiter-from-list '(1 2 3))))
      :to-equal
      '(1 2 3)))
  (it
    "collect over an empty iterator yields nil"
    (expect
      (cl-cc/runtime::rt-future-await
        (cl-cc/runtime::rt-aiter-collect (cl-cc/runtime::rt-aiter-from-list '())))
      :to-be-null))
  (it
    "rt-aiter-next reports the item with done-p nil, then nil with done-p t at the end"
    (let ((it (cl-cc/runtime::rt-aiter-from-list '(:only))))
      (multiple-value-bind (item done) (cl-cc/runtime::rt-future-await (cl-cc/runtime::rt-aiter-next it))
  ...
  (it
    "rt-aiter-map transforms every item"
    (expect
      (cl-cc/runtime::rt-future-await
      ...
      '(10 20 30)))
  (it
    "rt-aiter-filter keeps only matching items"
    (expect
      (cl-cc/runtime::rt-future-await
        (cl-cc/runtime::rt-aiter-collect
      ...
      '(2 4 6)))
  (it
    "rt-aiter-take limits to the first n items"
    (expect
      (cl-cc/runtime::rt-future-await
        (cl-cc/runtime::rt-aiter-collect
      ...
      '(1 2)))
  (it
    "rt-aiter-take of more than exists stops at the source's end"
    (expect
      (cl-cc/runtime::rt-future-await
        (cl-cc/runtime::rt-aiter-collect
          (cl-cc/runtime::rt-aiter-take (cl-cc/runtime::rt-aiter-from-list '(1 2)) 10)))
      :to-equal
      '(1 2)))
  (it
    "rt-async-for walks every item in order"
    (let ((acc nil))
      (cl-cc/runtime::rt-async-for
        (x (cl-cc/runtime::rt-aiter-from-list '(:a :b :c)))
        (push x acc))
    ...
    (let ((g (cl-cc/runtime::rt-make-async-generator)))
      ...
      (let ((f (cl-cc/runtime::rt-async-generator-next g)))
        ...
        (multiple-value-bind (item done) (cl-cc/runtime::rt-future-await f)
          ...
          (expect done :to-be-null)))))
  (it
    "next-then-yield registers a waiter that a later yield resolves"
    (let* ((g (cl-cc/runtime::rt-make-async-generator))
      ...
      (multiple-value-bind (item done) (cl-cc/runtime::rt-future-await f)
        ...
        (expect done :to-be-null))))
  (it
    "queued items are delivered in FIFO order"
    (let ((g (cl-cc/runtime::rt-make-async-generator)))
      ...
      (expect
        ...
        2)))
  (it
    "close marks the generator done; next then reports done-p"
    (let ((g (cl-cc/runtime::rt-make-async-generator)))
      (expect (cl-cc/runtime::rt-async-generator-close g) :to-be-truthy)
      (multiple-value-bind (item done) (cl-cc/runtime::rt-future-await (cl-cc/runtime::rt-async-generator-next g))
        (expect item :to-be-null)
        (expect done :to-be-truthy))))
  (it
    "close resolves any pending waiter with done-p true"
    (let* ((g (cl-cc/runtime::rt-make-async-generator))
           (f (cl-cc/runtime::rt-async-generator-next g)))
      (expect (cl-cc/runtime::rt-future-done-p f) :to-be-null)
      ...
      (multiple-value-bind (item done) (cl-cc/runtime::rt-future-await f)
        ...
        (expect done :to-be-truthy))))
  (it
    "yield after close signals an error"
    (let ((g (cl-cc/runtime::rt-make-async-generator)))
      (cl-cc/runtime::rt-async-generator-close g)
      (signals error (cl-cc/runtime::rt-async-yield g :x))))
  (it
    "a failed generator raises the recorded condition on next"
    (let ((g (cl-cc/runtime::rt-make-async-generator)))
      ...
      (signals error (cl-cc/runtime::rt-async-generator-next g)))))

(describe
  "lowered coroutines (async-generators.lisp)"
  (it
    "resumes stackless steps in order"
    (let ((coroutine
          (cl-cc/runtime:rt-lower-coroutine
            (list
              (lambda ()
                :first)
              (lambda ()
      ...
      (multiple-value-bind (value done) (cl-cc/runtime:rt-coroutine-resume coroutine)
        ...
        (expect done :to-be-truthy))))
  (it
    "selects stackful lowering for deep yield on call/cc targets"
    (let ((coroutine
          (cl-cc/runtime:rt-lower-coroutine
            (list
              (lambda ()
