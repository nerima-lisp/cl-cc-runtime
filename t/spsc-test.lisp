;;;; t/spsc-test.lisp — single-producer/single-consumer ring buffer (src/spsc.lisp).
(in-package :cl-cc-runtime/test)

(describe
  "spsc capacity sizing"
  (it
    "rounds capacity up to a power of two, minimum 2"
...
(describe
  ...
  (it
    "is empty and not full on creation"
  ...
  (it
    "try-pop on empty returns (nil nil)"
    (let ((q (rt-make-spsc-queue 4)))
      (multiple-value-bind (item ok) (rt-spsc-try-pop q)
        (expect item :to-be-null)
        (expect ok :to-be-null))))
  (it
    "try-push returns t and grows size; try-pop returns the item"
    (let ((q (rt-make-spsc-queue 4)))
      ...
      (expect (rt-spsc-empty-p q) :to-be-truthy)))
  (it
    "rejects a push into a full ring and accepts again after a pop"
  ...
  (it
    "reset clears contents and role ownership"
    (let ((q (rt-make-spsc-queue 4)))
...
(describe
  "spsc concurrent producer/consumer"
  (it
    "transfers all items in order with no loss or duplication"
    (let* ((n 20000)
          ...
          (sb-thread:make-thread
            (lambda ()
              (dotimes (i n)
                (rt-spsc-push q i :spin t)))))
           (consumer
          (sb-thread:make-thread
            (lambda ()
              (dotimes (i n)
                (setf (aref out i) (rt-spsc-pop q :spin t)))))))
...
(describe
  "spsc init"
  (it "rt-spsc-init returns t" (expect (rt-spsc-init) :to-be-truthy)))
