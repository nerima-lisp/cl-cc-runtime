;;;; t/image-core-test.lisp — Coverage for src/image-core.lisp: FR-1002/FR-1003
...
(defun %rt-core-test-path (name)
  (namestring
    (merge-pathnames
      (make-pathname :name name :type "core")
...
(defun %rt-core-register-test-roots ()
  (mapc
    #'cl-cc/runtime::rt-image-register-global
    '(*rt-core-test-root*
...
(it-sequential
  ...
  (let ((path (%rt-core-test-path "clcc-closure")))
    (setf *rt-core-test-closure* (let ((n 40))
        (lambda (x)
          (+ n x))))
    (cl-cc/runtime::rt-save-core path :compression :none)
    (setf *rt-core-test-closure* (lambda (x)
        x))
    (cl-cc/runtime::rt-load-core path)
