...
(deftest future-resolve-single-value
  "rt-future-resolve stores a value; rt-future-await returns it, done-p is set."
  (let ((f (cl-cc/runtime::rt-make-future)))
    (assert-false (cl-cc/runtime::rt-future-done-p f))
    (cl-cc/runtime::rt-future-resolve f :answer)
    (assert-true (cl-cc/runtime::rt-future-done-p f))
    (assert-eq :answer (cl-cc/runtime::rt-future-await f))))

(deftest future-resolve-multiple-values
  ...
  (let ((f (cl-cc/runtime::rt-make-future)))
    ...
    (multiple-value-bind (a b c) (cl-cc/runtime::rt-future-await f)
      (assert-= 1 a) (assert-= 2 b) (assert-= 3 c))))

(deftest future-await-when-already-resolved-returns-immediately
  "Awaiting an already-resolved future does not block."
  (let ((f (cl-cc/runtime::rt-make-future)))
    (cl-cc/runtime::rt-future-resolve f :ready)
    (assert-eq :ready (cl-cc/runtime::rt-future-await f :timeout 0.01))))

(deftest future-await-timeout-on-unresolved-returns-no-values
  "Awaiting a never-resolved future with a timeout returns (no values) once the
...
instantly."
  (let* ((f (cl-cc/runtime::rt-make-future))
    ...
    (assert-true (>= elapsed (* 0.5 timeout)))))

(deftest future-await-long-timeout-resolves-partway
  "A generous timeout does not cut the wait short: a future resolved partway
through the budget is awaited to its value, well before the deadline."
  (let* ((f (cl-cc/runtime::rt-make-future))
         (result-box (list nil))
         (start (get-internal-real-time))
    ...
    (let ((elapsed (/ (- (get-internal-real-time) start)
                      (float internal-time-units-per-second))))
      (assert-eq :in-time (first result-box))
      ;; Returned on resolution, nowhere near the 5s deadline.
      (assert-true (< elapsed 2.0)))))

(deftest future-all-futures-awaits-each
  "rt-all-futures returns its argument list after awaiting every future."
  (let ((fs (list (cl-cc/runtime::rt-make-future)
                  (cl-cc/runtime::rt-make-future)
                  (cl-cc/runtime::rt-make-future))))
    (loop for f in fs for i from 0 do (cl-cc/runtime::rt-future-resolve f i))
    (assert-eq fs (cl-cc/runtime::rt-all-futures fs))
    (assert-true (every #'cl-cc/runtime::rt-future-done-p fs))))

;;; ─── rt-future-then (scheduler-driven continuation) ─────────────────────────

(deftest future-then-chains-callback
  "rt-future-then spawns a green thread that awaits the source future and
resolves the derived future with the callback's result."
  ...
  (let* ((src (cl-cc/runtime::rt-make-future))
    ...
    (assert-true (cl-cc/runtime::rt-future-done-p derived))
    (assert-= 70 (cl-cc/runtime::rt-future-await derived))))

;;; ─── Genuine cross-thread synchronization ───────────────────────────────────

(deftest future-cross-thread-await-resolve
  "A future awaited on one OS thread is unblocked by a resolve on another,
proving the condition-variable handoff works across real threads."
  (let* ((f (cl-cc/runtime::rt-make-future))
         (result-box (list nil))
         (waiter (sb-thread:make-thread
                  (lambda ()
    ...
    (sb-thread:join-thread waiter)
    (assert-eq :from-other-thread (first result-box))))

;;; ─── Property: any resolved value round-trips through await ──────────────────

(cl-weave:it-property "future round-trips arbitrary resolved integers"
    ((n (cl-weave:gen-integer :min -1000 :max 1000)))
  (let ((f (cl-cc/runtime::rt-make-future)))
    (cl-cc/runtime::rt-future-resolve f n)
    (and (cl-cc/runtime::rt-future-done-p f)
