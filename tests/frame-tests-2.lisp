;;;; tests/frame-tests-2.lisp — additional vm-frame / frame-pool coverage (src/frame.lisp).
;;;; Complements tests/frame-tests.lisp: pool exhaustion/overflow, raw constructor
;;;; defaults, boundary register access, and reset of the closure/return-frame fields.
(in-package :cl-cc-runtime/test)

(describe "vm-frame construction (frame.lisp)"
  (it "a raw frame has 256 registers, all +val-nil+, and nil meta-fields"
    (let ((f (cl-cc/runtime::%make-vm-frame)))
      ...
      (expect (every (lambda (x) (= x cl-cc/runtime:+val-nil+))
                     (cl-cc/runtime::vm-frame-registers f))
              :to-be-truthy)))

  (it "the register array is a specialized (unsigned-byte 64) simple array"
    (let ((f (cl-cc/runtime::%make-vm-frame)))
      (expect (typep (cl-cc/runtime::vm-frame-registers f)
                     '(simple-array (unsigned-byte 64) (*)))
              :to-be-truthy)))

  (it "+frame-pool-size+ is 1024"
    (expect cl-cc/runtime::+frame-pool-size+ :to-be 1024)))

(describe "frame-pool exhaustion and overflow (frame.lisp)"
  (it "initialize-frame-pool stocks the pool to full"
    (cl-cc/runtime::initialize-frame-pool)
    (expect cl-cc/runtime::*frame-pool-top* :to-be cl-cc/runtime::+frame-pool-size+))

  (it "acquire on an exhausted pool allocates a fresh frame and leaves top at zero"
    (cl-cc/runtime::initialize-frame-pool)
  ...
  (it "release on a full pool drops the frame, leaving top unchanged"
    (cl-cc/runtime::initialize-frame-pool)
  ...
  (it "acquire decrements the pool top by one"
    (cl-cc/runtime::initialize-frame-pool)
...
(describe "register access boundaries (frame.lisp)"
  (it "registers 0 and 255 round-trip full 64-bit values"
    (let ((f (cl-cc/runtime::%make-vm-frame))
          (big (1- (ash 1 64))))
  ...
  (it-property "any register index stores and returns its value independently"
...
(describe "frame-reset clears object references (frame.lisp)"
  (it "reset nils out closure and return-frame"
    (let ((f (cl-cc/runtime::%make-vm-frame))
  ...
  (it "a released frame's object references are cleared on the next acquire"
    (cl-cc/runtime::initialize-frame-pool)
    (let ((f (cl-cc/runtime::frame-pool-acquire)))
