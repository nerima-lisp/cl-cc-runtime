...
;;;; implementations of SHA-256, SHA-512 (FIPS 180-4) and HMAC-SHA256
;;;; (RFC 2104). Operate on (vector (unsigned-byte 8)) octet vectors unless
;;;; noted otherwise. Base64 (RFC 4648) is in crypto-base64.lisp -- an
;;;; unrelated encoding, not a hash.
;;;;
;;;; RT-SHA256 and RT-SHA512's 64/80-round compression loops share the same
;;;; Davies-Meyer shape (differing in word width, mask, round count, K
;;;; table and sigma/ch/maj functions) and a shared macro could generate
;;;; both. Left unmerged on purpose: each function currently reads as a
;;;; direct transcription of its own FIPS 180-4 section, checkable
;;;; line-by-line against the spec, which is a real property to keep for
;;;; hash-function code specifically -- collapsing them behind a
;;;; parameterized macro would trade that auditability for a DRY
;;;; consolidation this file's few call sites do not need.
