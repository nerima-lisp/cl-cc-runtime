...
(defmacro rt-with-mutex ((m &key timeout) &body body)
  `(unwind-protect (progn (rt-mutex-lock ,m :timeout ,timeout) ,@body)
     (rt-mutex-unlock ,m)))
...
(defun rt-condition-wait (cv m &key timeout)
  (sb-thread:condition-wait (rt-condition-variable-waitqueue cv)
                            (rt-mutex-host-mutex m)
                            :timeout (or timeout 1e6)))
...
(defun rt-semaphore-wait (s &key timeout)
  (rt-with-mutex ((rt-semaphore-m s) :timeout timeout)
    (loop while (<= (rt-semaphore-count s) 0)
          do (rt-condition-wait (rt-semaphore-c s) (rt-semaphore-m s)))
    ...
    (let ((g (rt-barrier-gen b)))
      ...
      (if (= (rt-barrier-count b) (rt-barrier-total b))
          ...
          (progn (loop while (= g (rt-barrier-gen b))
                       do (rt-condition-wait (rt-barrier-c b) (rt-barrier-m b)
                                             :timeout timeout))
                 nil)))))
(defstruct rt-once (done-p nil) (m (rt-make-mutex)))
(defun rt-make-once () (make-rt-once))
(defun rt-once-call (o fn)
  (rt-with-mutex ((rt-once-m o))
    (unless (rt-once-done-p o)
      (setf (rt-once-done-p o) (cons t (funcall fn)))))
  (cdr (rt-once-done-p o)))
    ...
    (unwind-protect
         (progn
           (loop while (or (rt-rwlock-writer rw) (> (rt-rwlock-readers rw) 0))
                 do (rt-condition-wait (rt-rwlock-writers-ok rw) (rt-rwlock-mutex rw)
                                       :timeout timeout))
